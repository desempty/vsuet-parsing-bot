...
CONFIG = {
    ...
    "CHECK_INTERVAL": 2700, # Интервал между проверками рейтинга в 45 минут
...
DICT_SUBJECT = {
    ...
    "Администрирование вычислительных сетей": "254036",
    "Информационная безопасность": "254037",
    "Менеджмент": "254038",
    "Общая физическая подготовка": "254039",
    "Объектно-ориентированные системы программирования": "254040",
    "Операционные системы": "254041",
    "Программирование на платформе 1С:Предприятие": "254042",
    "Прогрессивное технологическое оборудование": "254043",
    "Продукты питания животного происхождения": "254044",
    "Проектный менеджмент": "254045"
}
...
# Глобальный ограничитель запросов к сайту
REQUEST_DELAY = 1.2  # максимум ~0.8 rps
...
# 1.6. Функция проверки сессии
def is_session_valid(chat_id):
    with data_lock:
        data = user_selected_data.get(chat_id)
        return data and data.get("version") == CURRENT_SESSION_VERSION
        ...
        try:
            ...
            for subject_name in subjects:
                ...
                try:
                    ...
                    logger.warning(f"Сайт недоступен для {student_id}: {e}")
                    continue
                ...
                with data_lock:
                    previous_ratings.setdefault(chat_id, {})[subject_name] = current_data
...
@bot.message_handler(func=lambda message: message.text in ["Отмена", "Вернуться назад", "вернуться назад", "отмена"])
...
def handle_subject_choice_after_id(message):
    ...
    if not is_session_valid(message.chat.id):
        bot.send_message(message.chat.id, "Бот обновился. Нажмите 'Начать', 'Отмена', или пропишите команду '/start'")
        return
